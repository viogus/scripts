...
- `contacts-pinyin.swift` — 单文件，放在 repo 根目录
...
检查 `familyName` 和 `givenName` 是否包含 CJK 统一表意文字（U+4E00–U+9FFF）。任一字段含中文即判定为中文联系人。
